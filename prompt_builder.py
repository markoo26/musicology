...
def create_prompt_builder_graph(config: dict):
    class PromptBuilderState(TypedDict):
        ...
        attributes_to_collect: list[str] = Field(default=config["SONG_ATTRIBUTES"])
        prompt_attributes: dict[str, str] = Field(default_factory={})
        validation_attempts: int = Field(default=0)
        messages: list[str] = Field(default_factory=[])
    ...
    def collect_attributes_node(state: PromptBuilderState) -> PromptBuilderState:
        ...
        user_input = interrupt(
            f"\nPlease describe the {current_attribute}:"
        )
